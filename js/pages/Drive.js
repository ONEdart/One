// js/pages/Drive.js
// Main Drive UI - Fully Integrated with DriveManager.js
...
export default {
  ...
  data() {
    return {
      ...
      // Items data (populated from DriveManager)
      items: [],
  ...
  computed: {
    currentFolder() {
      ...
      return this.driveManager ? this.driveManager.getFolderInfo(this.currentFolderId) : null
    },
    
    currentFolderName() {
      return this.currentFolder ? this.currentFolder.name : 'Drive'
    },
    
    breadcrumbs() {
      if (!this.currentFolderId || this.currentFolderId === 'root') return ['Drive']
      ...
      while (current) {
        ...
        current = current.parentId ? this.driveManager.getFolderInfo(current.parentId) : null
      }
      
      crumbs.unshift('Drive')
      return crumbs
    },
  ...
  methods: {
      ...
      try {
        ...
        setInterval(() => {
          if (this.driveManager) {
            this.driveManager.saveToLocalStorage()
          }
    ...
    async loadFolderContents() {
      ...
      try {
        const contents = this.driveManager.getFolderContents(this.currentFolderId, false)
    ...
    formatFileForUI(file) {
      return {
        ...
        starred: file.metadata?.starred || false,
    ...
    goBack() {
      if (this.currentFolderId && this.currentFolderId !== 'root') {
        const currentFolder = this.driveManager.getFolderInfo(this.currentFolderId)
        this.currentFolderId = currentFolder?.parentId || 'root'
        this.clearSelection()
    ...
    showFilePreview(file) {
      ...
      const fileInfo = this.driveManager.getFileInfo(file.id)
      const accountInfo = fileInfo?.account || { name: 'Unknown Account' }
        ...
        <div class="bg-white border border-slate-300 rounded-2xl p-6 max-w-md w-full shadow-2xl">
          ...
          <div class="flex items-center space-x-4 mb-6">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="text-xs text-slate-500 mt-1">Stored in: ${accountInfo.name}</div>
            </div>
          </div>
          
          <div class="space-y-3 mb-6">
            <div class="flex justify-between text-sm">
              <span class="text-slate-600">File ID:</span>
              <span class="font-mono text-slate-900">${file.id}</span>
            </div>
            <div class="flex justify-between text-sm">
              <span class="text-slate-600">Created:</span>
              <span class="text-slate-900">${file.date}</span>
            </div>
            <div class="flex justify-between text-sm">
              <span class="text-slate-600">Type:</span>
              <span class="text-slate-900">${this.getTypeDisplay(file.type)}</span>
            </div>
          </div>
          
          <div class="flex justify-end space-x-3">
            <button onclick="this.closest('.fixed').remove()" class="px-4 py-2 border border-slate-300 text-slate-700 text-sm font-semibold rounded-lg hover:bg-slate-50 transition-colors">
              Close
            </button>
            <button onclick="this.downloadFile('${file.id}')" class="px-4 py-2 bg-slate-900 text-white text-sm font-semibold rounded-lg hover:bg-slate-800 transition-colors">
              Download
            </button>
          </div>
        </div>
      ...
      window.downloadFile = async (fileId) => {
        const result = await this.downloadFiles([fileId])
        if (result.success) {
          modal.remove()
    ...
    async toggleStar(itemId) {
      ...
      if (item && item.type !== 'folder') {
        this.driveManager.toggleStar(itemId)
        await this.loadFolderContents()
      }
    },
    ...
    async createNewFolder() {
      ...
      const result = this.driveManager.createFolder(
        this.newFolderName.trim(),
        this.currentFolderId,
        {
          color: ['blue', 'green', 'purple', 'yellow', 'pink', 'indigo', 'teal'][
    ...
    async moveSelectedItems(targetFolderId) {
      ...
      const result = this.driveManager.moveItems(selectedIds, targetFolderId)
    ...
    async processUpload(files, targetFolderId = this.currentFolderId) {
      ...
      // Process upload with DriveManager
      try {
        const result = await this.driveManager.uploadFiles(
          fileList.map(file => ({
            name: file.name,
            size: file.size,
            type: file.type
    ...
    async handleDrop(e) {
      ...
      if (isInternalDrag) {
        ...
        try {
          ...
          if (data.type === 'move') {
            const result = this.driveManager.moveItems(data.items, this.currentFolderId)
            if (result.success) {
              await this.loadFolderContents()
    ...
    async handleFolderDrop(folder, e) {
      ...
      if (isInternalDrag) {
        ...
        try {
          const data = JSON.parse(e.dataTransfer.getData('application/json'))
          if (data.type === 'move') {
            const result = this.driveManager.moveItems(data.items, folder.id)
            if (result.success) {
              await this.loadFolderContents()
    ...
    async downloadSelectedFiles() {
      ...
      if (result.success) {
        this.showNotification(`Preparing ${result.files.length} file(s) for download`)
        
        // In a real app, this would trigger actual downloads
        // For now, we'll just show a notification
        console.log('Files to download:', result.files)
      } else {
        this.showNotification(result.error, 'error')
      }
    },
    ...
    async deleteSelectedItems() {
      ...
      const result = this.driveManager.deleteItems(selectedIds, false) // false = move to trash
        ...
        image: `
          <div class="w-12 h-12 bg-purple-50 rounded-lg flex items-center justify-center border-2 border-purple-200">
            <svg class="w-6 h-6 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
          </div>
        `,
        video: `
    ...
    showNotification(message, type = 'success') {
      ...
      notification.className = `drive-notification fixed top-4 right-4 px-6 py-3 rounded-lg shadow-lg z-50 transform transition-all duration-300 ${
        type === 'error' ? 'bg-red-500 text-white' : 'bg-slate-900 text-white'
      }`
        ...
        <div class="bg-white border border-slate-300 rounded-2xl p-6 max-w-2xl w-full shadow-2xl">
          ...
          <div class="mb-6">
            <div class="flex justify-between items-center mb-2">
              <span class="text-slate-700 font-medium">Virtual Pool</span>
              <span class="font-semibold text-slate-900">${stats.formatted.usedSpace} / ${stats.formatted.totalSpace}</span>
            </div>
            <div class="h-3 bg-slate-200 rounded-full overflow-hidden">
              <div class="h-full bg-slate-900 transition-all duration-300" 
                   style="width: ${stats.virtualPool.usagePercentage.toFixed(1)}%"></div>
            </div>
            <div class="text-xs text-slate-500 mt-1 text-right">
              ${stats.virtualPool.usagePercentage.toFixed(1)}% used
            </div>
          </div>
            ...
            ${accountStats.accounts.map(account => `
              <div class="p-3 border border-slate-300 rounded-lg">
                <div class="flex justify-between items-center mb-2">
                  ...
                  <div class="text-sm font-medium ${account.usagePercentage >= 90 ? 'text-red-600' : account.usagePercentage >= 80 ? 'text-orange-600' : 'text-slate-600'}">
                    ${(account.usedSpace / (1024 * 1024 * 1024)).toFixed(1)} GB / ${(account.totalSpace / (1024 * 1024 * 1024)).toFixed(0)} GB
                  </div>
                </div>
                <div class="h-2 bg-slate-200 rounded-full overflow-hidden">
                  <div class="h-full ${account.usagePercentage >= 90 ? 'bg-red-500' : account.usagePercentage >= 80 ? 'bg-orange-500' : 'bg-slate-600'}" 
                       style="width: ${account.usagePercentage.toFixed(1)}%"></div>
                </div>
                <div class="flex justify-between text-xs text-slate-500 mt-1">
                  <span>${account.fileCount} files</span>
                  <span>${account.usagePercentage.toFixed(1)}% used</span>
                </div>
              </div>
            `).join('')}
          </div>
          ...
          <div class="flex justify-end mt-6">
            <button onclick="this.closest('.fixed').remove()" class="px-5 py-2.5 bg-slate-900 text-white text-sm font-semibold rounded-lg hover:bg-slate-800 transition-colors">
              Close
            </button>
          </div>
        </div>
        ...
        <div>
          ...
          >
            ...
            >
              ...
              >
                <svg class="w-5 h-5 transition-transform" :class="item.starred ? 'text-yellow-500 fill-yellow-500' : 'text-slate-300'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div class="mt-4">
                ...
                <div class="flex items-center justify-between text-sm mt-3">
                  <div class="text-slate-600" :class="getSizeColor(item.size)">
                    {{ item.size }}
                  </div>
                  <div class="text-xs text-slate-400">{{ item.date }}</div>
                </div>
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td class="py-4 px-4 sm:px-6">
                      ...
                      >
                        <svg class="w-5 h-5" :class="item.starred ? 'text-yellow-500 fill-yellow-500' : 'text-slate-300'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"></path>
                        </svg>
                      </button>
                    </td>
          ...
          <div class="mb-6">
            ...
            <div class="space-y-2 max-h-64 overflow-y-auto">
              ...
              >
                ...
                <div>
                  <div class="font-semibold text-slate-900">Drive (Root)</div>
                  <div class="text-xs text-slate-500">Move to main drive</div>
                </div>
              </div>
              
              <!-- Available folders (excluding current folder and its descendants) -->
              <div 
                v-for="folder in folders.filter(f => {
                  // Exclude selected folders
                  if (selectedItems.has(f.id)) return false
